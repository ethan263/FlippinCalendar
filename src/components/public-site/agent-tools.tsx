  ...
  const tools: AgentClientTools = {
    ...
    book_appointment: async (parameters) => {
      try {
        ...
        const booking = await createPublicBookingAction({
          siteSlug,
          offeringId: offering._id,
    ...
    lookup_appointment: async (parameters) => {
      try {
        const result = await lookupPublicBookingAction({
          siteSlug,
    ...
    reschedule_appointment: async (parameters) => {
      try {
        ...
        const result = await reschedulePublicBookingAction({
          siteSlug,
          confirmationCode: requiredText(
            parameters.confirmation_code,
            "confirmation_code",
    ...
    cancel_appointment: async (parameters) => {
      try {
        const result = await cancelPublicBookingAction({
          siteSlug,
          confirmationCode: requiredText(
            parameters.confirmation_code,
            "confirmation_code",
